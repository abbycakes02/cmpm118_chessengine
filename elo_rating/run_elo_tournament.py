...
MODEL_PATH = os.path.join(project_root, "backend_engine", "ml", "models", "session_1764582902", "chess_valuenet_32ch_3resblocks.pth") # 32 channels, 3 blocks - VALUE HEAD (working model)
MAX_DEPTH = 6   # Increased from 5 - with GPU batching we can search deeper!
TIME_LIMIT = 3  # Increased from 3 - give engine more time with batching speedup

# Tournament Settings
GAMES_PER_LEVEL = 5  # More games = more accurate rating (10-20 recommended)
STOCKFISH_LEVELS = [2, 3, 4]  # Start easier to find competitive level
SAVE_PGN = True  # Save games to PGN file for analysis
# Different PGN files for NN vs non-NN tournaments
if USE_NN:
    PGN_OUTPUT = os.path.join(current_dir, "minimax_nn_vs_stockfish.pgn")
else:
    PGN_OUTPUT = os.path.join(current_dir, "minimax_basic_vs_stockfish.pgn")
    ...
    try:
        if USE_NN:
            my_engine = MinimaxEngine(
                ...
                channels=32,  # Must match the model architecture - 32 for value head
